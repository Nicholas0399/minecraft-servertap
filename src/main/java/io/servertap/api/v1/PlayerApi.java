...
import io.servertap.api.v1.models.TikTokStatus;
...
public class PlayerApi {
    ...
    @OpenApi(
            path = "/v1/players/{uuid}/tiktok-status",
            methods = {HttpMethod.GET},
            summary = "Gets TikTok stream status for a player",
            tags = {"Player"},
            headers = {
                    @OpenApiParam(name = "key")
            },
            ...
            responses = {
                    @OpenApiResponse(status = "200", content = @OpenApiContent(from = TikTokStatus.class)),
                    @OpenApiResponse(status = "404", content = @OpenApiContent(from = Error.class))
            }
    )
    public void playerTikTokStatus(Context ctx) {
        String uuid = ctx.pathParam("uuid");
        if (uuid.isEmpty()) throw new BadRequestResponse(Constants.PLAYER_UUID_MISSING);
        ...
        // Получаем конфиг через Bukkit API
        FileConfiguration config = Bukkit.getPluginManager().getPlugin("ServerTap").getConfig();
        String tiktokUser = config.getString("tiktok.streamers." + playerUUID.toString());
        
        if (tiktokUser == null) {
            throw new NotFoundResponse("TikTok account not registered for this player");
        }

        TikTokStatus status = new TikTokStatus();
        status.setUsername(tiktokUser);
        status.setStreaming(config.getBoolean("tiktok.streamers." + tiktokUser + ".streaming", false));
        status.setManual(config.getBoolean("tiktok.streamers." + tiktokUser + ".manual", false));
        status.setLastChecked(config.getLong("tiktok.streamers." + tiktokUser + ".lastChecked", 0));
        status.setLive(config.getBoolean("tiktok.streamers." + tiktokUser + ".isLive", false));
        
        ctx.json(status);
    }

    @OpenApi(
        path = "/v1/tiktok/status",
        methods = {HttpMethod.PUT},
        summary = "Update TikTok stream status manually",
        tags = {"Player"},
        headers = {
            @OpenApiParam(name = "key")
        },
        requestBody = @OpenApiRequestBody(content = @OpenApiContent(from = TikTokStatus.class)),
        responses = {
            @OpenApiResponse(status = "200", content = @OpenApiContent(from = TikTokStatus.class)),
            @OpenApiResponse(status = "400", content = @OpenApiContent(from = Error.class)),
            @OpenApiResponse(status = "404", content = @OpenApiContent(from = Error.class))
        }
    )
    public void updateTikTokStatus(Context ctx) {
        TikTokStatus status = ctx.bodyAsClass(TikTokStatus.class);
        if (status.getUsername() == null) {
            throw new BadRequestResponse("Username is required");
        }

        FileConfiguration config = Bukkit.getPluginManager().getPlugin("ServerTap").getConfig();
        String path = "tiktok.streamers." + status.getUsername();
        
        if (!config.contains(path)) {
            throw new NotFoundResponse("TikTok account not registered");
        }

        if (status.isStreaming() != null) {
            config.set(path + ".streaming", status.isStreaming());
        }
        if (status.isManual() != null) {
            config.set(path + ".manual", status.isManual());
        }
        if (status.getLastChecked() != null) {
            config.set(path + ".lastChecked", status.getLastChecked());
        }
        if (status.isLive() != null) {
            config.set(path + ".isLive", status.isLive());
        }

        Bukkit.getPluginManager().getPlugin("ServerTap").saveConfig();
        ctx.json(status);
    }

    @OpenApi(
        path = "/v1/tiktok/streamers",
        methods = {HttpMethod.GET},
        summary = "Get list of registered TikTok streamers",
        tags = {"Player"},
        headers = {
            @OpenApiParam(name = "key")
        },
    ...
    public void listTikTokStreamers(Context ctx) {
        FileConfiguration config = Bukkit.getPluginManager().getPlugin("ServerTap").getConfig();
        ArrayList<TikTokStatus> streamers = new ArrayList<>();
        
        ConfigurationSection streamersSection = config.getConfigurationSection("tiktok.streamers");
        if (streamersSection != null) {
            for (String key : streamersSection.getKeys(false)) {
                if (key.length() > 36) continue; // Skip UUID keys
                
                TikTokStatus status = new TikTokStatus();
                status.setUsername(key);
                status.setStreaming(config.getBoolean("tiktok.streamers." + key + ".streaming", false));
                status.setManual(config.getBoolean("tiktok.streamers." + key + ".manual", false));
                status.setLastChecked(config.getLong("tiktok.streamers." + key + ".lastChecked", 0));
                status.setLive(config.getBoolean("tiktok.streamers." + key + ".isLive", false));
                
                streamers.add(status);
            }
        }
